...
export class MapComponent implements AfterViewInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      (changes["vehicles"] && this.map) ||
      (changes["customers"] && this.map)
    ) {
      this.updateMarkers();
    }
  }
  ...
  private async updateMarkers(): Promise<void> {
    if (isPlatformBrowser(this.platformId)) {
      ...
      this.vehicles.forEach((vehicle) => {
        ...
        let [xt, yt] = [customer?.coordX, customer?.coordY];

        let clamp = (x: number, min: number, max: number) => {
          return Math.min(Math.max(x, min), max);
        };

        let [xi, yi] = interpolatePosition(
          vehicle.coordX,
          vehicle.coordY,
          xt!,
          yt!,
          1 - clamp(vehicle.remainingTravelTime / 300, 0, 1)
        );

        const marker = L.marker([xi, yi], {
          icon: vehicleIcon,
        })
          .addTo(this.map)
      ...
      this.customers.forEach((customer) => {
        const customerMarker = L.marker([customer.coordX, customer.coordY], {
          icon: customerIcon,
        })
          .addTo(this.map)
